...
trait Tables {
  ...
  case class AliasRow(id: Int, domainId: Int, address: String = "", goto: String, active: Boolean = true)
  /** GetResult implicit for fetching AliasRow objects using plain SQL queries */
  implicit def GetResultAliasRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Boolean]): GR[AliasRow] = GR{
    prs => import prs._
    AliasRow.tupled((<<[Int], <<[Int], <<[String], <<[String], <<[Boolean]))
  }
  ...
  class Alias(_tableTag: Tag) extends Table[AliasRow](_tableTag, "alias") {
    def * = (id, domainId, address, goto, active) <> (AliasRow.tupled, AliasRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(domainId), Rep.Some(address), Rep.Some(goto), Rep.Some(active)).shaped.<>({r=>import r._; _1.map(_=> AliasRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
   ...
   *  @param updatedAt Database column updated_at SqlType(DATETIME), Default(None)
   *  @param active Database column active SqlType(BIT), Default(true) */
  case class DomainRow(id: Int, domain: String = "", description: String = "", active: Boolean = true)
  /** GetResult implicit for fetching DomainRow objects using plain SQL queries */
  implicit def GetResultDomainRow(implicit e0: GR[Int], e1: GR[String], e2: GR[Boolean]): GR[DomainRow] = GR{
    prs => import prs._
    DomainRow.tupled((<<[Int], <<[String], <<[String], <<[Boolean]))
  }
  ...
  class Domain(_tableTag: Tag) extends Table[DomainRow](_tableTag, "domain") {
    def * = (id, domain, description, active) <> (DomainRow.tupled, DomainRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(domain), Rep.Some(description), Rep.Some(active)).shaped.<>({r=>import r._; _1.map(_=> DomainRow.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
